...
This descriptive statistics analysis has been produced to provide greater insight into the education and children's social care background of children who had been cautioned or sentenced for an offence. However, this analysis does not imply there is a causal link between these characteristics and offending.  
...
ethnic_major_chart <- ggplot(ethnic_major_long, aes(x=value, y=`Ethnic Group Major`, fill = `Ethnic Group Major`)) +
  ...
  facet_wrap(~Pupil_group) + theme(strip.background = element_blank()) +
  xlim(0,105) +
...
```{r SEN chart, echo=FALSE,fig.dim = c(8, 6), fig.cap="**Special Educational Needs (SEN) of all pupils by offending and pupil group, for those that were ever recorded with SEN, for pupils matched to KS4 academic years 2012/13, 2013/14 and 2014/15. [Source: DfE-MoJ data share, table: ...]**"}
SEN_chart
```
...
```{r SEMH timing chart, echo=FALSE,fig.dim = c(8, 6), eval=NA %!in% SEMH_timing_for_chart$`% with SEMH timing`, fig.cap="**The first term children had been recorded with SEMH, relative to the timing of their first serious violence offence, for pupils matched to KS4 academic years 2012/13, 2013/14 and 2014/15. [Source: DfE-MoJ data share, table: ...]**"}
SEMH_timing_chart
```
