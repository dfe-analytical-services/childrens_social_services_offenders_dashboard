...
pre_prop <- function() {
  ...
  No_pupils_sclLA <- No_pupils_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  No_pupils_hmLA <- No_pupils_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)

  scl_with_dif_hm <- scl_with_dif_hm %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA) %>%
    select(LA, indicator, count_all_dif, count_any_dif, count_sv_dif, prop_count_all_dif, prop_count_any_dif, prop_count_sv_dif)
  hm_with_dif_scl <- hm_with_dif_scl %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA) %>%
    select(LA, indicator, count_all_dif, count_any_dif, count_sv_dif, prop_count_all_dif, prop_count_any_dif, prop_count_sv_dif)

  previous_offence_sclLA <- previous_offence_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA) %>%
    select(!sv)
  previous_offence_hmLA <- previous_offence_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA) %>%
    select(!sv)
  ...
  info_table <- info_table %>% select(
    LA, indicator, all, offenders, sv, prop_off, prop_sv, count_previous, prop_previous,
  ...
  Gender_hmLA <- Gender_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  Gender_sclLA <- Gender_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  # Join above tables
  ...
  Gender_all <- Gender %>%
    select(LA, indicator, prop_count_M, prop_count_F) %>%
    mutate(group = "All Pupils") %>%
  ...
  Gender_any <- Gender %>%
    select(LA, indicator, prop_any_count_M, prop_any_count_F) %>%
    mutate(group = "Any Offence") %>%
    pivot_longer(cols = c(prop_any_count_M, prop_any_count_F), names_to = "Gender", values_to = "perc") %>%
    mutate(Gender = ifelse(str_detect(Gender, "_M") == TRUE, "Male", "Female"))

  Gender_sv <- Gender %>%
    select(LA, indicator, prop_sv_count_M, prop_sv_count_F) %>%
    mutate(group = "Serious Violence Offence") %>%
  ...
  # Create an indicator for home or school and change Home/school_LA to LA
  Ethnicity_major_hmLA <- Ethnicity_major_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  Ethnicity_major_sclLA <- Ethnicity_major_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  # Join above tables
  ...
  Ethnicity_all <- Ethnicity %>%
    select(LA, indicator, EthnicGroupMajor, prop_all) %>%
    mutate(group = "All Pupils") %>%
    pivot_longer(cols = c(prop_all), values_to = "perc") %>%
    select(!name)

  Ethnicity_any <- Ethnicity %>%
    select(LA, indicator, EthnicGroupMajor, prop_off) %>%
    mutate(group = "Any Offence") %>%
    pivot_longer(cols = c(prop_off), values_to = "perc") %>%
    select(!name)

  Ethnicity_sv <- Ethnicity %>%
    select(LA, indicator, EthnicGroupMajor, prop_sv) %>%
    mutate(group = "Serious Violence Offence") %>%
  ...
  # Make Ethnicity 1 long data frame (combine hmLA and sclLA)
  # Create an indicator for home or school and change Home/school_LA to LA
  Everflag_hmLA <- Everflag_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  Everflag_sclLA <- Everflag_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  # Join above tables
  ...
  # Make FSM table long
  FSM_all <- FSM %>%
    select(LA, indicator, prop_count_FSM) %>%
    mutate(group = "All Pupils") %>%
    pivot_longer(cols = c(prop_count_FSM), values_to = "perc") %>%
    select(!name)

  FSM_any <- FSM %>%
    select(LA, indicator, any_prop_count_FSM) %>%
    mutate(group = "Any Offence") %>%
    pivot_longer(cols = c(any_prop_count_FSM), values_to = "perc") %>%
    select(!name)

  FSM_sv <- FSM %>%
    select(LA, indicator, sv_prop_count_FSM) %>%
    mutate(group = "Serious Violence Offence") %>%
  ...
  FSM_LHS <- Everflag %>%
    select(LA, indicator, sv_prop_count_FSM) %>%
    mutate(prop_sv_not_FSM = 100 - sv_prop_count_FSM)
  FSM_RHS <- Everflag %>%
    select(LA, indicator, also_sv_prop_count_FSM) %>%
    mutate(not_also_sv_prop_count_FSM = 100 - also_sv_prop_count_FSM)
  ...
  # Create an indicator for home or school and change Home/school_LA to LA
  Age_first_offence_hmLA <- Age_first_offence_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  Age_first_offence_sclLA <- Age_first_offence_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  # Join above tables
  ...
  # Make age long table
  age_offence_any <- age_offence %>%
    select(LA, indicator, OffenceStartAge, prop_off) %>%
    mutate(group = "Any Offence") %>%
    pivot_longer(cols = c(prop_off), values_to = "perc") %>%
    select(!name)

  age_offence_sv <- age_offence %>%
    select(LA, indicator, OffenceStartAge, prop_sv) %>%
    mutate(group = "Serious Violence Offence") %>%
    pivot_longer(cols = c(prop_sv), values_to = "perc") %>%
    select(!name)
  ...
  write_xlsx(
    list(
      info_table = info_table, Gender = Gender, Ethnicity = Ethnicity,
  ...
  # KS2
  # Create an indicator for home or school and change Home/school_LA to LA
  KS2_attain_sclLA <- KS2_attain_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  KS2_attain_hmLA <- KS2_attain_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  # Join above tables
  ...
  KS2_attain <- KS2_attain %>%
    select(
      LA, indicator, propcount_EngL4orabove, propcount_MatL4orabove,
    ...
    pivot_longer(
      cols = c(
        propcount_EngL4orabove, propcount_MatL4orabove,
        propcount_EngL4orabove_offenders, propcount_MatL4orabove_offenders,
        propcount_EngL4orabove_SV_offenders, propcount_MatL4orabove_SV_offenders
      ),
  ...
  # KS4
  KS4_attain_sclLA <- KS4_attain_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  KS4_attain_hmLA <- KS4_attain_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  # Join above tables
  ...
  KS4_attain <- KS4_attain %>%
    select(
      LA, indicator, propcount_anypass, propcount_5AG, propcount_5AC,
      propanycount_anypass, propanycount_5AG, propanycount_5AC,
      propsvcount_anypass, propsvcount_5AG, propsvcount_5AC
    ) %>%
    pivot_longer(
      cols = c(
        propcount_anypass, propcount_5AG, propcount_5AC,
        propanycount_anypass, propanycount_5AG, propanycount_5AC,
        propsvcount_anypass, propsvcount_5AG, propsvcount_5AC
      ),
      names_to = "tempSubject", values_to = "perc"
  ...
  EverPAPAUO <- Everflag %>%
    select(
      LA, indicator, prop_count_PA, any_prop_count_PA, sv_prop_count_PA,
      prop_count_PAUO, any_prop_count_PAUO, sv_prop_count_PAUO
    ) %>%
    pivot_longer(
      cols = c(
        prop_count_PA, any_prop_count_PA, sv_prop_count_PA,
        prop_count_PAUO, any_prop_count_PAUO, sv_prop_count_PAUO
      ),
  ...
  # PA waffle
  PA_waffle <- Everflag %>%
    select(LA, indicator, sv_prop_count_PA, also_sv_prop_count_PA) %>%
    mutate(
      prop_sv_not_PA = 100 - sv_prop_count_PA,
      not_also_sv_prop_count_PA = 100 - also_sv_prop_count_PA
    )

  # PA/uo Timing
  PAPAUO_timing_sclLA <- PAPAUO_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  PAPAUO_timing_hmLA <- PAPAUO_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  # Join above tables
  ...
  PAPAUO_timing <- PAPAUO_timing %>%
    select(
      LA, indicator, prop_PA_Before_SV, prop_PASame_Term, prop_PA_After_SV,
      prop_PAUO_Before_SV, prop_PAUOSame_Term, prop_PAUO_After_SV
    ) %>%
    pivot_longer(
      cols = c(
        prop_PA_Before_SV, prop_PASame_Term, prop_PA_After_SV,
        prop_PAUO_Before_SV, prop_PAUOSame_Term, prop_PAUO_After_SV
      ),
      names_to = "tempAbsence", values_to = "perc"
    ) %>%
  ...
  # Add tables currently needed to environment
  list2env(alldata[c(
    "First_sus_timing_sclLA", "First_sus_timing_hmLA", "First_excl_timing_sclLA", "First_excl_timing_hmLA",
    "Closest_sus_timing_sclLA", "Closest_sus_timing_hmLA", "Closest_excl_timing_sclLA", "Closest_excl_timing_hmLA"
  )], envir = .GlobalEnv)

  # Ever sus / excl
  EverSusExcl <- Everflag %>%
    select(
      LA, indicator, prop_count_Sus, any_prop_count_Sus, sv_prop_count_Sus,
      prop_count_Excl, any_prop_count_Excl, sv_prop_count_Excl
    ) %>%
    pivot_longer(
      cols = c(
        prop_count_Sus, any_prop_count_Sus, sv_prop_count_Sus,
        prop_count_Excl, any_prop_count_Excl, sv_prop_count_Excl
      ),
      names_to = "tempAbsence", values_to = "perc"
    ) %>%
  ...
  # sus / excl waffle
  SusExcl_waffle <- Everflag %>%
    select(
      LA, indicator, sv_prop_count_Excl, also_sv_prop_count_Excl,
      sv_prop_count_Sus, also_sv_prop_count_Sus
    ) %>%
    mutate(
      prop_sv_not_Excl = 100 - sv_prop_count_Excl,
  ...
  First_sus_timing_sclLA <- First_sus_timing_sclLA %>%
    mutate(indicator = "School", SusExcl = "Suspended", Time = "First") %>%
    rename(LA = SCHOOL_LA, prop_absencefirst_all = prop_susfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_susfirst))
  First_sus_timing_hmLA <- First_sus_timing_hmLA %>%
    mutate(indicator = "Home", SusExcl = "Suspended", Time = "First") %>%
    rename(LA = HOME_LA, prop_absencefirst_all = prop_susfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_susfirst))
  First_excl_timing_sclLA <- First_excl_timing_sclLA %>%
    mutate(indicator = "School", SusExcl = "Permanently excluded", Time = "First") %>%
    rename(LA = SCHOOL_LA, prop_absencefirst_all = prop_exclfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_exclfirst))
  First_excl_timing_hmLA <- First_excl_timing_hmLA %>%
    mutate(indicator = "Home", SusExcl = "Permanently excluded", Time = "First") %>%
    rename(LA = HOME_LA, prop_absencefirst_all = prop_exclfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_exclfirst))
  Closest_sus_timing_sclLA <- Closest_sus_timing_sclLA %>%
    mutate(indicator = "School", SusExcl = "Suspended", Time = "Closest") %>%
    rename(LA = SCHOOL_LA, prop_absencefirst_all = prop_susfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_susfirst))
  Closest_sus_timing_hmLA <- Closest_sus_timing_hmLA %>%
    mutate(indicator = "Home", SusExcl = "Suspended", Time = "Closest") %>%
    rename(LA = HOME_LA, prop_absencefirst_all = prop_susfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_susfirst))
  Closest_excl_timing_sclLA <- Closest_excl_timing_sclLA %>%
    mutate(indicator = "School", SusExcl = "Permanently excluded", Time = "Closest") %>%
    rename(LA = SCHOOL_LA, prop_absencefirst_all = prop_exclfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_exclfirst))
  Closest_excl_timing_hmLA <- Closest_excl_timing_hmLA %>%
    mutate(indicator = "Home", SusExcl = "Permanently excluded", Time = "Closest") %>%
    rename(LA = HOME_LA, prop_absencefirst_all = prop_exclfirst_all) %>%
    select(!contains(c("count")) & !c(prop_svfirst, prop_exclfirst))

  fst.clst_SusExcl <- bind_rows(
    First_sus_timing_sclLA, First_sus_timing_hmLA, First_excl_timing_sclLA, First_excl_timing_hmLA,
    Closest_sus_timing_sclLA, Closest_sus_timing_hmLA, Closest_excl_timing_sclLA, Closest_excl_timing_hmLA
  )
  ...
  fst.clst_SusExcl <- fst.clst_SusExcl %>%
    pivot_longer(
      cols = c(prop_svfirst_all, prop_absencefirst_all),
      names_to = "key",
      values_to = "Perc"
    ) %>%
  ...
  fst.clst_SusExcl <- fst.clst_SusExcl %>%
    group_by(LA, indicator, SusExcl, Time) %>%
    arrange(LA, indicator, SusExcl, Time, time_group)
  ...
  # Ever AP
  EverAP <- Everflag %>%
    select(LA, indicator, prop_count_AP, any_prop_count_AP, sv_prop_count_AP) %>%
    pivot_longer(
      cols = c(prop_count_AP, any_prop_count_AP, sv_prop_count_AP),
  ...
  # AP waffle
  AP_waffle <- Everflag %>%
    select(LA, indicator, sv_prop_count_AP, also_sv_prop_count_AP) %>%
    mutate(
      prop_sv_not_AP = 100 - sv_prop_count_AP,
      not_also_sv_prop_count_AP = 100 - also_sv_prop_count_AP
    )

  # AP Timing
  AP_timing_sclLA <- AP_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  AP_timing_hmLA <- AP_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  # Join above tables
  ...
  AP_timing <- AP_timing %>%
    select(LA, indicator, prop_AP_Before_SV, prop_APSame_Term, prop_AP_After_SV) %>%
    pivot_longer(
      cols = c(prop_AP_Before_SV, prop_APSame_Term, prop_AP_After_SV),
      names_to = "tempAP", values_to = "perc"
    ) %>%
  ...
  # Add tables currently needed to environment
  list2env(alldata[c(
    "SEN_sclLA", "SEN_hmLA", "SEN_timing_sclLA", "SEN_timing_hmLA", "SEMH_timing_sclLA", "SEMH_timing_hmLA",
    "EHCP_timing_sclLA", "EHCP_timing_hmLA"
  )], envir = .GlobalEnv)
  ...
  EverSEN <- EverSEN_all %>%
    ...
    mutate(
      SEN_type = ifelse(str_detect(tempSEN, "EHCP") == TRUE, "EHCP",
  ...
  SEN_waffle <- EverSEN_all %>%
    ...
    mutate(
      prop_not_SEN_support_SV = 100 - prop_SEN_support_SV,
  ...
  EHCP_waffle <- EverSEN_all %>%
    ...
    mutate(
      prop_not_EHCP_SV = 100 - prop_EHCP_SV,
      not_prop_also_EHCP_SV = 100 - prop_also_EHCP_SV
    )
  rm(EverSEN_all)

  # Timing of SEN
  SEN_timing_sclLA <- SEN_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  SEN_timing_hmLA <- SEN_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  SEN_timing <- bind_rows(SEN_timing_sclLA, SEN_timing_hmLA)
  rm(SEN_timing_sclLA, SEN_timing_hmLA)
  SEN_timing <- SEN_timing %>%
    select(LA, indicator, prop_SEN_Before_SV, prop_SENSame_Term, prop_SEN_After_SV) %>%
    pivot_longer(
      cols = c(prop_SEN_Before_SV, prop_SENSame_Term, prop_SEN_After_SV),
  ...
  # Timing of EHCP
  EHCP_timing_sclLA <- EHCP_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  EHCP_timing_hmLA <- EHCP_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  EHCP_timing <- bind_rows(EHCP_timing_sclLA, EHCP_timing_hmLA)
  rm(EHCP_timing_sclLA, EHCP_timing_hmLA)

  EHCP_timing <- EHCP_timing %>%
    select(LA, indicator, prop_EHCP_Before_SV, prop_EHCPSame_Term, prop_EHCP_After_SV) %>%
    pivot_longer(
      cols = c(prop_EHCP_Before_SV, prop_EHCPSame_Term, prop_EHCP_After_SV),
      names_to = "Temp", values_to = "perc"
    ) %>%
    mutate(Timing = ifelse(str_detect(Temp, "Before") == TRUE, "Before the first serious violence offence",
      ifelse(str_detect(Temp, "Same") == TRUE, "In the same term as the first serious violence offence",
  ...
  # Timing of SEMH
  SEMH_timing_sclLA <- SEMH_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  SEMH_timing_hmLA <- SEMH_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  SEMH_timing <- bind_rows(SEMH_timing_sclLA, SEMH_timing_hmLA)
  rm(SEMH_timing_sclLA, SEMH_timing_hmLA)

  SEMH_timing <- SEMH_timing %>%
    select(LA, indicator, prop_SEMH_Before_SV, prop_SEMHSame_Term, prop_SEMH_After_SV) %>%
    pivot_longer(
      cols = c(prop_SEMH_Before_SV, prop_SEMHSame_Term, prop_SEMH_After_SV),
      names_to = "Temp", values_to = "perc"
    ) %>%
    mutate(Timing = ifelse(str_detect(Temp, "Before") == TRUE, "Before the first serious violence offence",
      ifelse(str_detect(Temp, "Same") == TRUE, "In the same term as the first serious violence offence",
  ...
  write_xlsx(
    list(
      KS2_attain = KS2_attain, KS4_attain = KS4_attain, EverPAPAUO = EverPAPAUO,
  ...
  # Create an indicator for home or school and change Home/school_LA to LA
  CiN_CLA_sclLA <- CiN_CLA_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  CiN_CLA_hmLA <- CiN_CLA_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  # Join above tables
  ...
  CIN_waffle <- EverCINCLA_all %>%
    ...
    mutate(
      propsv_count_not_CIN = 100 - propsv_count_CIN,
  ...
  # Create an indicator for home or school and change Home/school_LA to LA
  CLA_timing_sclLA <- CLA_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  CLA_timing_hmLA <- CLA_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  CLA_timing <- bind_rows(CLA_timing_sclLA, CLA_timing_hmLA)
  rm(CLA_timing_sclLA, CLA_timing_hmLA)

  CSC_timing_sclLA <- CSC_timing_sclLA %>%
    mutate(indicator = "School") %>%
    rename(LA = SCHOOL_LA)
  CSC_timing_hmLA <- CSC_timing_hmLA %>%
    mutate(indicator = "Home") %>%
    rename(LA = HOME_LA)
  ...
  CSC_timing <- CSC_timing %>%
    ...
    select(!tempCINCLA)
  # CLA
  CLA_timing <- CLA_timing %>%
    select(LA, indicator, prop_CLA_Before_SV, prop_CLASame_Term, prop_CLA_After_SV) %>%
    pivot_longer(
      cols = c(prop_CLA_Before_SV, prop_CLASame_Term, prop_CLA_After_SV),
      names_to = "tempCINCLA", values_to = "perc"
    ) %>%
    mutate(Timing = ifelse(str_detect(tempCINCLA, "Before") == TRUE, "Before the first serious violence offence",
      ifelse(str_detect(tempCINCLA, "Same") == TRUE, "In the same term as the first serious violence offence",
  ...
  write_xlsx(
    list(EverCINCLA = EverCINCLA, CIN_waffle = CIN_waffle, CSC_timing = CSC_timing),
